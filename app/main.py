...
def _unifi_from_payload(payload: dict) -> tuple[UniFiClient | None, dict | None]:
    cfg = all_settings()
    url = str(payload.get("unifi_url") or cfg.get("unifi_url") or "").strip()
    api_key = str(payload.get("unifi_api_key") or "").strip() or (get_secret("unifi_api_key") or "")
    verify_ssl = _bool(payload.get("unifi_verify_ssl", cfg.get("unifi_verify_ssl")), False)
...
def startup() -> None:
    initialise(); start_monitoring()


@app.get("/api/health")
def api_health() -> dict:
    return {"status": "ok", "version": VERSION, "environment": CONFIG.environment, "database": str(DB_PATH)}
...
async def api_save_settings(request: Request) -> dict:
    ...
    unifi_api_key = str(payload.get("unifi_api_key", "")).strip(); discord_webhook = str(payload.get("discord_webhook", "")).strip()
    if unifi_api_key: set_secret("unifi_api_key", unifi_api_key)
    if discord_webhook: set_secret("discord_webhook", discord_webhook)
...
async def api_test_unifi(request: Request) -> dict:
    ...
    return error or client.test_connection()  # type: ignore[union-attr]
...
async def api_test_ups(request: Request) -> dict:
    payload = await request.json(); ups_type = str(payload.get("ups_type", "nutpi_http")); host = str(payload.get("ups_host", "")).strip(); status_path = str(payload.get("nutpi_status_path", "/api/nutpi/status.cgi")).strip()
    if not host: return {"ok": False, "message": "Enter the UPS/NUT host or IP address"}
    if ups_type == "nutpi_http": return NutPiHttpClient(host, status_path).test_connection()
...
async def api_test_ping(request: Request) -> dict:
    payload = await request.json(); return _ping(str(payload.get("ping_target", "")))
...
def api_network_changes() -> dict:
    con = connect()
    try:
        rows = con.execute("SELECT id, ts, category, summary, details FROM network_changes ORDER BY id DESC LIMIT 25").fetchall(); return {"items": [dict(row) for row in rows]}
    finally: con.close()


@app.post("/api/network-changes")
async def api_add_network_change(request: Request) -> dict:
    payload = await request.json(); category = str(payload.get("category", "General")).strip() or "General"; summary = str(payload.get("summary", "")).strip(); details = str(payload.get("details", "")).strip()
    if not summary: return {"ok": False, "message": "Enter a summary of the change"}
    con = connect()
    try:
        cur = con.execute("INSERT INTO network_changes(category, summary, details) VALUES (?, ?, ?)", (category, summary, details)); con.commit(); return {"ok": True, "id": cur.lastrowid}
    finally: con.close()
...
def api_system_info() -> dict:
    ...
    return {"version": VERSION, "environment": CONFIG.environment, "database": str(DB_PATH), "database_exists": DB_PATH.exists(), "python": platform.python_version(), "platform": platform.system(), "hostname": platform.node(), "encryption": enc}


@app.get("/api/dashboard/summary")
def api_dashboard_summary() -> dict:
    cfg = all_settings(); summary: dict[str, object] = {"version": VERSION, "setup_complete": _bool(cfg.get("setup_complete")), "internet": {"enabled": _bool(cfg.get("isp_enabled")), "ok": None}, "unifi": {"enabled": _bool(cfg.get("unifi_enabled")), "ok": None}, "ups": {"enabled": _bool(cfg.get("ups_enabled")), "ok": None}, "wifi": {"enabled": _bool(cfg.get("unifi_enabled")), "ok": None}}
    if summary["internet"]["enabled"]:  # type: ignore[index]
        ping = _ping(str(cfg.get("ping_target", "1.1.1.1"))); summary["internet"] = {"enabled": True, "ok": ping.get("ok", False), "target": cfg.get("ping_target", "1.1.1.1"), "provider": cfg.get("isp_provider", "")}
    if summary["unifi"]["enabled"]:  # type: ignore[index]
        key = get_secret("unifi_api_key") or ""; url = str(cfg.get("unifi_url", ""))
        if key and url:
            test = UniFiClient(url, key, _bool(cfg.get("unifi_verify_ssl"))).test_connection(); summary["unifi"] = {"enabled": True, **test}; summary["wifi"] = {"enabled": True, "ok": test.get("ok", False)}
        else:
            summary["unifi"] = {"enabled": True, "ok": False, "message": "Configuration required"}; summary["wifi"] = {"enabled": True, "ok": False}
    if summary["ups"]["enabled"]:  # type: ignore[index]
        if str(cfg.get("ups_type", "nutpi_http")) == "nutpi_http":
            test = NutPiHttpClient(str(cfg.get("ups_host", "")), str(cfg.get("nutpi_status_path", "/api/nutpi/status.cgi"))).test_connection(); summary["ups"] = {"enabled": True, **test}
        else: summary["ups"] = {"enabled": True, "ok": False, "message": "Direct NUT not connected"}
    return summary
...
def dashboard(request: Request) -> HTMLResponse:
    return HTMLResponse(templates.get_template("dashboard.html").render(request=request, version=VERSION, page="dashboard", title="Dashboard"))


@app.get("/settings", response_class=HTMLResponse)
def settings(request: Request) -> HTMLResponse:
    return HTMLResponse(templates.get_template("settings.html").render(request=request, version=VERSION, page="settings", title="Settings"))
