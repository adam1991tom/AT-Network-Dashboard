...
async def authentication(request: Request, call_next):
    ...
    public = path.startswith("/static/") or path in {"/api/health", "/login", "/setup-admin"}
    if public:
...
def logout_api(request: Request):
    logout(request.cookies.get(COOKIE_NAME)); response=JSONResponse({"ok":True}); response.delete_cookie(COOKIE_NAME); return response
...
async def api_test_unifi(request: Request) -> dict:
    payload=await request.json(); client,error=_unifi_from_payload(payload); return error or client.test_connection()

@app.post("/api/settings/test/speedtest")
async def api_test_speedtest(request: Request) -> dict:
    payload=await request.json(); client,error=_unifi_from_payload(payload); return error or client.run_speedtest()
...
def api_system_info() -> dict:
    return {"version":VERSION,"environment":CONFIG.environment,"database":str(DB_PATH),"database_exists":DB_PATH.exists(),"python":platform.python_version(),"platform":platform.system(),"hostname":platform.node(),"encryption":encryption_status(),"authentication":True}
...
async def api_update_apply(request: Request) -> dict:
    data=await request.json(); return request_update(str(data.get("channel","stable")), str(data.get("target") or ""))
