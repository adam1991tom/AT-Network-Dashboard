    ...
    try:
        ...
        if _bool(cfg.get("ups_enabled")) and str(cfg.get("ups_type", "nutpi_http")) == "nutpi_http":
            try:
                ...
                con.execute(
                    """INSERT INTO ups_history(ts,connected,status,load_pct,input_voltage,output_voltage,battery_voltage,input_frequency,runtime_seconds)
                       VALUES (?,?,?,?,?,?,?,?,?)""",
        ...
        if _bool(cfg.get("unifi_enabled")):
            ...
            if key and url:
                try:
                    ...
                    con.execute(
                        """INSERT INTO gateway_history(ts,uptime,cpu,memory,temperature,wan_up,wan_ip,link_speed,rx_errors,tx_errors,rx_dropped,tx_dropped,rx_rate,tx_rate)
                           VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                    ...
                    if isinstance(speed, dict):
                        ...
                        if epoch_ms and epoch_ms != _last_speedtest_epoch and speed.get("download") is not None:
                            ...
                            con.execute(
                                """INSERT OR IGNORE INTO speedtest_history(ts,epoch_ms,download,upload,latency,interface_name,wan_group,source)
                                   VALUES (?,?,?,?,?,?,?,'unifi')""",
                    ...
                    for ap in snapshot.get("aps", []):
                        for radio in ap.get("radios", []):
                            con.execute(
                                """INSERT INTO wifi_history(ts,device_id,ap_name,band,channel,width,retries,utilization,clients,satisfaction,tx_power)
                                   VALUES (?,?,?,?,?,?,?,?,?,?,?)""",
    ...
    try:
        ...
        aps = con.execute(
            """SELECT w.* FROM wifi_history w
               JOIN (SELECT ap_name,band,MAX(id) AS max_id FROM wifi_history GROUP BY ap_name,band) x ON w.id=x.max_id
