...
def ping_sample(target: str) -> dict[str, Any]:
    try:
        ...
        return {
            ...
            "latency": latency,
            "packet_loss": loss,
            "online": 1 if result.returncode == 0 else 0,
        }
    except Exception:
    ...
    try:
        if _bool(cfg.get("isp_enabled"), True):
            ...
            con.execute(
                ...
                (ts, target, ping["latency"], ping["packet_loss"], ping["online"]),
            )

        if _bool(cfg.get("ups_enabled")) and str(cfg.get("ups_type", "nutpi_http")) == "nutpi_http":
            try:
                data = NutPiHttpClient(
                    str(cfg.get("ups_host", "")),
                    str(cfg.get("nutpi_status_path", "/api/nutpi/status.cgi")),
                ).status()
                con.execute(
                    """
                    INSERT INTO ups_history(ts,connected,status,load_pct,input_voltage,output_voltage,battery_voltage,input_frequency)
                    VALUES (?,?,?,?,?,?,?,?)
                    """,
                    (
                        ts,
                        1,
                        data.get("ups.status"),
                        _float(data.get("ups.load")),
                        _float(data.get("input.voltage")),
                        _float(data.get("output.voltage")),
                        _float(data.get("battery.voltage")),
                        _float(data.get("input.frequency")),
                    ),
                )
            except Exception:
                ...
                try:
                    ...
                    con.execute(
                        ...
                        INSERT INTO gateway_history(
                            ts,uptime,cpu,memory,temperature,wan_up,wan_ip,link_speed,
                            rx_errors,tx_errors,rx_dropped,tx_dropped,rx_rate,tx_rate
                        ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                        """,
                        (
                            ts,
                            gw.get("uptime"), gw.get("cpu"), gw.get("memory"), gw.get("temperature"),
                            1 if gw.get("wan_up") else 0, gw.get("wan_ip"), gw.get("link_speed"),
                            gw.get("rx_errors"), gw.get("tx_errors"), gw.get("rx_dropped"), gw.get("tx_dropped"),
                            gw.get("rx_rate"), gw.get("tx_rate"),
                        ),
                    )
                        ...
                        if epoch_ms and epoch_ms != _last_speedtest_epoch and speed.get("download") is not None:
                            ...
                            con.execute(
                                ...
                                INSERT OR IGNORE INTO speedtest_history(
                                    ts,epoch_ms,download,upload,latency,interface_name,wan_group,source
                                ) VALUES (?,?,?,?,?,?,?,'unifi')
                                """,
                                (
                                    speed_ts, epoch_ms, speed.get("download"), speed.get("upload"), speed.get("latency"),
                                    speed.get("interface_name"), speed.get("wan_group") or "WAN",
                                ),
                            )
                        ...
                        for radio in ap.get("radios", []):
                            con.execute(
                                ...
                                INSERT INTO wifi_history(
                                    ts,device_id,ap_name,band,channel,width,retries,utilization,clients,satisfaction,tx_power
                                ) VALUES (?,?,?,?,?,?,?,?,?,?,?)
                                """,
                                (
                                    ts, ap.get("device_id"), ap.get("name"), radio.get("band"), radio.get("channel"),
                                    radio.get("width"), radio.get("retries"), radio.get("utilization"), radio.get("clients"),
                                    radio.get("satisfaction"), radio.get("tx_power"),
                                ),
                            )
...
def start_monitoring() -> None:
    ...
    with _worker_lock:
        if _worker_started:
            return
        thread = threading.Thread(target=_worker, name="at-network-monitor", daemon=True)
        thread.start()
...
def _history(table: str, hours: int, columns: str = "*") -> list[dict[str, Any]]:
    ...
    try:
        rows = con.execute(
            f"SELECT {columns} FROM {table} WHERE datetime(ts) >= datetime('now', ?) ORDER BY datetime(ts) ASC",
            (f"-{hours} hours",),
        ).fetchall()
...
def ups_history(hours: int) -> list[dict[str, Any]]:
    return _history("ups_history", hours, "ts,connected,status,load_pct,input_voltage,output_voltage,battery_voltage,input_frequency")
...
def live_snapshot() -> dict[str, Any]:
    ...
    try:
        ...
        aps = con.execute(
            ...
            JOIN (SELECT ap_name,band,MAX(id) AS max_id FROM wifi_history GROUP BY ap_name,band) x
              ON w.id=x.max_id
            ORDER BY w.ap_name,w.band
            """
        ).fetchall()
        return {
            "ping": one("ping_history"),
            "speedtest": one("speedtest_history"),
            "gateway": one("gateway_history"),
            "ups": one("ups_history"),
            "wifi": [dict(row) for row in aps],
        }
    finally:
        con.close()
