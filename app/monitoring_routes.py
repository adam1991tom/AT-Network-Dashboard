...
def _preserve_extremes(rows: list[dict], fields: tuple[str, ...], max_rows: int = 1800) -> list[dict]:
    """Downsample dense telemetry while preserving spikes/drops and endpoints."""
...
def _wifi_rows(hours: int, limit: int = 0) -> list[dict]:
    rows = _rows(
        "wifi_history",
        "id,ts,device_id,ap_name,band,channel,width,retries,utilization,clients,satisfaction,tx_power",
        hours,
    )
    if limit:
        return rows[-limit:]
...
def ping(hours: int = Query(24, ge=1, le=8760)):
    rows = _rows("ping_history", "id,ts,target,latency,packet_loss,online", hours)
    return _preserve_extremes(rows, ("latency", "packet_loss"), 1800)
...
def speedtests(hours: int = Query(24, ge=1, le=8760)):
    ...
    # Speed tests are sparse and evidential: keep every genuine row unless the DB contains
    # an extreme historical volume, then preserve min/max performance in each bucket.
...
def incidents_api(limit: int = Query(1000, ge=1, le=5000)):
    ...
    try:
        return {"items": [dict(r) for r in con.execute(
            "SELECT id,incident_type,incident_key,category,device,severity,started_at,ended_at,last_seen_at,summary,details,active,operator_note,fault_reference FROM incidents ORDER BY active DESC,id DESC LIMIT ?",
            (limit,),
        ).fetchall()]}
    finally:
        con.close()
...
def incidents_page(request: Request):
    ...
    try:
        items = [dict(r) for r in con.execute(
            "SELECT id,incident_type,incident_key,category,device,severity,started_at,ended_at,last_seen_at,summary,details,active,operator_note,fault_reference FROM incidents ORDER BY active DESC,id DESC LIMIT 2000"
        ).fetchall()]
    finally:
        con.close()
