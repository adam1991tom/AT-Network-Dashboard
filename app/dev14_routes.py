...
router=APIRouter(tags=['dev14']);VERSION='2.0.0-dev14'
@router.get('/api/health')
def health():return {'status':'ok','version':VERSION,'environment':CONFIG.environment,'database':str(DB_PATH)}
@router.get('/api/system/info')
def system_info():return {'version':VERSION,'environment':CONFIG.environment,'database':str(DB_PATH),'database_exists':DB_PATH.exists(),'python':platform.python_version(),'platform':platform.system(),'hostname':platform.node(),'encryption':encryption_status(),'authentication':True,'schema':'dev14'}
...
def build_info():
 cfg=all_settings();return {'version':VERSION,'schema':'dev14','update_channel':cfg.get('update_channel','stable'),'theme':cfg.get('theme','dark'),'accent':cfg.get('accent','green')}
