...
def system_backup():
    ...
    return Response(backup_bytes(),media_type='application/zip',headers={'Content-Disposition':f'attachment; filename="AT-Network-Dashboard-backup-{stamp}.zip"'})
...
async def system_restore(request:Request):
    ...
    try:
        ...
        if 'network-dashboard.sql' not in names:return JSONResponse({'ok':False,'message':'Invalid AT Network Dashboard backup'},status_code=400)
        sql=z.read('network-dashboard.sql').decode('utf-8')
        ...
        return {'ok':True,'message':'Backup restored. Restart the dashboard containers to complete restore.'}
...
def export_csv(dataset:str,hours:int=Query(168,ge=1,le=8760)):
    tables={'speedtests':'speedtest_history','ping':'ping_history','gateway':'gateway_history','wifi':'wifi_history','ups':'ups_history','incidents':'incidents'}
...
async def incident_note(incident_id:int,request:Request):
    p=await request.json();note=str(p.get('note') or '').strip();reference=str(p.get('reference') or '').strip()
    con=connect()
    try:
        row=con.execute('SELECT id FROM incidents WHERE id=?',(incident_id,)).fetchone()
        if not row:return JSONResponse({'ok':False,'message':'Incident not found'},status_code=404)
...
def build_info():
    cfg=all_settings()
    return {'version':'2.0.0-dev14','schema':'dev14','update_channel':cfg.get('update_channel','stable'),'theme':cfg.get('theme','dark'),'accent':cfg.get('accent','green')}
